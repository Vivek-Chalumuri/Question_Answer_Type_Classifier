...
def train_conv_net(datasets,
    ...
    f1 = file('classifier_state.pkl', 'wb')
    cPickle.dump(classifier.__getstate__(), f1, protocol=cPickle.HIGHEST_PROTOCOL)
    f1.close()
 
    f2 = file('conv_layer_state.pkl', 'wb')
    conv_params = []
    for i, conv_layer in enumerate(conv_layers):
	conv_params.append(conv_layer.__getstate__())
    cPickle.dump(conv_params, f2, protocol=cPickle.HIGHEST_PROTOCOL)
    f2.close()  
...
if __name__=="__main__":
    ...
    for i in [0]:
        ...
        perf, params = train_conv_net(datasets_tensor,
                              U,
                              lr_decay=0.95,
                              filter_hs=[3,4,5],
                              conv_non_linear="relu",
