...
def predict_conv_net(datasets,
    ...
    zero_vec_tensor = T.vector(dtype='float64')
    zero_vec = np.zeros(img_w,dtype='float64')
    #### initialization
    ...
    for i in xrange(len(filter_hs)):
        layer0_input = Words[T.cast(x.flatten(),dtype="int64")].reshape((x.shape[0],1,x.shape[1],Words.shape[1]*x.shape[-1]))[:,:,:,0:img_w*filter_hs[i]]
    ...
    for i in xrange(len(filter_sib)):
        layer0_input_sib = Words[T.cast(x.flatten(),dtype="int64")].reshape((x.shape[0],1,x.shape[1],Words.shape[1]*x.shape[-1]))[:,:,:,sib_templet[i]]
    ...
    for i, conv_layer in enumerate(conv_layers):
        if i < len(filter_hs):
            test_layer0_input = Words[T.cast(x.flatten(),dtype="int64")].reshape((test_size,1,img_h,Words.shape[1]*x.shape[-1]))[:,:,:,0:img_w*filter_hs[i]]
        else:
            test_layer0_input = Words[T.cast(x.flatten(),dtype="int64")].reshape((test_size,1,img_h,Words.shape[1]*x.shape[-1]))[:,:,:,sib_templet[i-len(filter_hs)]] 
